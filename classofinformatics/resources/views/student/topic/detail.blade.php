    ...
    <div class="block-header">
        <div class="row clearfix mb-4">
            <div class="col-md-6 col-sm-12">
                <a href="{{ url('/student/subject/' . $subject['id'] . '/course/' . $course['id'] . '/topic') }}"
                    class="text-white"><i class="icon-arrow-left text-white mr-2"></i>Kembali</a>
                <nav aria-label="breadcrumb" class="mt-4">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a class="text-white" href="{{ route('dashboard') }}">Daftar
                                Pelajaran</a></li>
                        <li class="breadcrumb-item"><a class="text-white"
                                href="{{ url('/student/subject/' . $subject['id'] . '/course/') }}">List Materi</a></li>
                        <li class="breadcrumb-item"><a class="text-white"
                                href="{{ url('/student/subject/' . $subject['id'] . '/course/' . $course['id'] . '/topic') }}">List
                                Bab</a></li>
                        <li class="breadcrumb-item active text-dark" aria-current="page">Detail</li>
                    </ol>
                </nav>
                <h5 class="text-white font-weight-bold text-capitalize mt-2">{{ $subject['name'] }} |
                    {{ $course['description'] }} | {{ $topic['name'] }}</h5>
            </div>
        </div>
        <div class="pb-3">
                ...
                <div class="row-clearfix mt-5">
                    ...
                    <div class="mt-3">
                        <a href=""
                            ...
                            <div class="d-flex align-items-center justify-content-center w35 bg-blue-2 rounded-circle cursor-pointer ml-2"
                                data-toggle="tooltip" data-placement="top" title="materi"><i
                                    class="fa fa-puzzle-piece text-white"></i></div>
                            <div class="ml-3">
                                <p class="text-dark text-uppercase text-dark pt-3">...</p>
                            </div>
                        </a>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        function renderActivity(data) {
            ...
            htmlExamMain = `
            <h5 class="color-black font-weight-bold text-capitalize">daftar Ujian</h5>
            <a class="color-black font-weight-bold">Terdapat <span class="text-white">${data.total_exam}</span> Ujian!</a>
            ${htmlExam}
        `
        ...
        function confirmExam(activityId, name, finish) {
            if (finish === true) {
                swal(`Anda sudah mengerjakan Ujian ${name}!`)
            } else {
                swal({
                    title: `Mulai Ujian ${name}?`,
                    ...
                    window.location.href =
                        `{{ url('/student/subject/${subject.id}/course/${course.id}/topic/${topic.id}/activity/${activityId}') }}`
                });
            }
        }
    </script>
